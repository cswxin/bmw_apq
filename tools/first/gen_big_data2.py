...
def write_data(term):
    ...
    tmpl_file = os.path.join(settings.RESOURCES_ROOT, u'big_data/2012数据大表_template2.xls')
    xls_file = os.path.join(settings.RESOURCES_ROOT, u'big_data/%s_数据大表2.xls' % term.name_cn)
    ...
    try:
        ...
        audi_startline = 5
        audi_area = {}
        areastrs = ['East', 'South', 'West', 'North']
        for dealer in audi_dealers:
            area = dealer.parent.parent
            values = audi_area.get(area.name)
            if values is None:
                values = []
            values.append(dealer)
            audi_area[area.name] = values
        
        for key in  areastrs:
            values = audi_area.get(key)
            for dealer in values:
    ...
    #                term.save()
        ...
        for dealer in lexus_dealers:
            area = dealer.parent.parent
            values = lexus_area.get(area.name)
            if values is None:
                values = []
            values.append(dealer)
            lexus_area[area.name] = values
        
        for key in  areastrs:
            values = lexus_area.get(key)
            for dealer in values:
                papers = Paper.objects.filter(dealer=dealer, project__id=4, paper_type='GFK', term=term, status=100)
                if papers and len(papers) > 0:
    #                dealers = []
    ...
    #                term.save()
        ...
        for dealer in benz_dealers:
            area = dealer.parent.parent
            values = benz_area.get(area.name)
            if values is None:
                values = []
            values.append(dealer)
            benz_area[area.name] = values
        
        for key in  areastrs:
            values = benz_area.get(key)
            for dealer in values:
                papers = Paper.objects.filter(dealer=dealer, project__id=4, paper_type='GFK', term=term, status=100)
                if papers and len(papers) > 0:
    #                dealers = []
...
def gen_excel_sheet_com(dealer, ss, startline, project_id, q_dict, paper_type, term):
    ...
    sheet.Range(r).Value = dealer.parent.parent.name
