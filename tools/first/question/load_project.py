...
def add_question_qa_cn(project):
    import xlrd
    xlsfile = os.path.join(settings.RESOURCES_ROOT, u"first/QA/2012 APQ_Q'naire_CN_BMW_完整版_0309.xls")
...
#        print texts
        ...
        if len(texts) >= 7:
            cids = texts[2].strip()
            q_desc = texts[9].strip()
            q_addon = texts[10].strip()
            a_desc = texts[11].strip()
            
            try:
                question = Question.objects.get(cid=cids, project=project)
...
def add_question_qa_en():
    #QuestionQA.objects.all().delete()
    import xlrd
    xlsfile = os.path.join(settings.RESOURCES_ROOT, u'first/QA/2011_questionnaire_Q__A_Q1_EN.xls')
    book = xlrd.open_workbook(xlsfile)
    _read_qa_from_xls(book, chinese=False)
...
def _read_qa_from_xls(book, chinese=True):
    sh = book.sheet_by_index(0)
    #print sh.name, sh.nrows, sh.ncols
    for rx in range(sh.nrows):
        texts = sh.row_values(rx)
        ...
        if len(texts) >= 6:
            ...
            cids = cids.replace(u'．', '')
            try:
                question = Question.objects.get(cid=cids)
            except Question.DoesNotExist:
                #print 'no question',cids
                continue
