...
#        create_dealer(code, name_cn, abbr_cn, has_child, dealertype, name_en=None, abbr_en=None, province_cn=None, city_cn=None, address=None, tel=None, level=None):        
        ...
        term.dealers.add(d)
#        if create:
        _add_user(dealer_code, dealer_code, '%s15' % dealer_code, [DEALER_GROUP], [SHOW_ALL_PAGE], d)
...
#            tel = tel.strip()
        ...
        if isinstance(xq_code, (int, float)):
            xq_code = '%s' % int(xq_code)
        xq_name_cn = texts[9].strip()

        #new_old = texts[13].strip()
        new_old = u"老店"
        if None in [area, province, city, dealer_code]:
            print u"第%s行有错误" % (rx + 1)
            continue
...
#            jtdealer, create = create_dealer(jt_code, jt_name_cn, jt_name_cn, True, dt, name_en=jt_name_en, parent=dealer_nation, jt_parent=dealer_nation, level=6)
        ...
        term.dealers.add(d)
#        if create:
        if dealer_code in ['35649', '36422', '36557', '36868', '37032', '36558', '38046', '38543', '38045']:
            _add_user('%s_M' % dealer_code, '%s_M' % dealer_code, '%s15_M' % dealer_code, [DEALER_GROUP], [SHOW_ALL_PAGE], d)
        else:
            _add_user(dealer_code, dealer_code, '%s15' % dealer_code, [DEALER_GROUP], [SHOW_ALL_PAGE], d)
...
def init_dealer():
    ...
    xlsfile = os.path.join(settings.SITE_ROOT, 'doc', 'dealer', u'APQ Dealer List for Q1 2015-to Nanyang-完整权限.xls')
