...
class Pinyin(object):
    ...
    def get_pinyin(self, chars=u"你好吗"):
        ...
        for char in chars:
            ...
            if len(chars)> index+1:
                if char==u'重' and chars[index+1:index+2]==(u'庆'):
                    poz = 1
                elif char==u'沈' and chars[index+1:index+2]==(u'阳'):
                    poz = 1
                
            try:
                result.append(self.dict[key].split(" ")[poz].strip()[:-1])
            except:
                result.append(char)
    ...
    def get_pinyin_city(self, chars=u"北方的的"):
        return self.get_pinyin(chars).capitalize().replace('rbo',' RBO')

if __name__ =="__main__":
    t = Pinyin()
    print t.get_pinyin(u'沈阳')
    print t.get_pinyin_city(u'北京RBO')
    print t.get_pinyin_name(u'重庆')